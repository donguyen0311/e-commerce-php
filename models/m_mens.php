...
class M_mens extends database
{
	...
	public function Doc_san_pham_nam($min_price, $max_price, $sort, $limit, $vi_tri = -1) {
		if (!empty($sort)) {
		...
		$sql = "SELECT s.*,l.noi_dung_khuyen_mai FROM san_pham s LEFT JOIN san_pham_khuyen_mai l ON s.ma_san_pham=l.ma_san_pham WHERE ma_loai_cha=9 AND (don_gia BETWEEN ? AND ?)" . $sap_xep . $gioi_han;
		$this->setQuery($sql);
